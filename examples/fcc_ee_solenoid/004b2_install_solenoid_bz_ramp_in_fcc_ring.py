"""Near-copy of 004b_install_solenoids_in_fcc_ring.py: installs the standard
SplineBoris main/compensation solenoids at every IP except RAMP_IP_NAME,
where the Bz-ramp-perturbed main-solenoid template (built by
004a2_build_solenoid_bz_ramp.py) is installed instead. Compensation solenoids,
correctors, and knob wiring are identical to 004b at every IP -- only the
main-solenoid field content at one IP differs.
"""
import argparse
...
bz_ramp_line_data = xt.json.load(INPUT_SOLENOID_LINES_BZ_RAMP_JSON)
main_solenoid_bz_ramp_template = xt.Line.from_dict(
    bz_ramp_line_data['lines']['main_solenoid'])
print(
    f"Bz-ramp main-solenoid template: ramp_amp="
    f"{bz_ramp_line_data['metadata']['ramp_amp']}, "
    f"ramp_slope_T_per_m={bz_ramp_line_data['metadata']['ramp_slope_T_per_m']:.6g}"
    f" -- will be installed at {RAMP_IP_NAME} only")
...
for ip_name in IP_NAMES:
    ...
    ip_main_template = (
        main_solenoid_bz_ramp_template if ip_name == RAMP_IP_NAME
        else main_solenoid_template)
    print(
        f'Installing SplineBoris solenoids and correctors around {ip_name}'
        + (' [Bz-ramp main solenoid]' if ip_name == RAMP_IP_NAME else ''))
...
plt.savefig(HERE / 'temp_bz_ramp_installed_bs.png', dpi=120)
print(f"Saved plot to {HERE / 'temp_bz_ramp_installed_bs.png'}")
