...
def _build_emitt_cases(tag, order_tag_str=""):
    """Build the EMITT_CASES list for a given field_tag (e.g. '2T', '3T') and
...
DEFAULT_EMITT_CASE_NAMES = ["sb_on", "varsol_on"]
...
def _select_cases(case_names, cases_by_name):
    if not case_names:
        return [cases_by_name[name] for name in DEFAULT_EMITT_CASE_NAMES]
...
def main():
    ...
    emitt_cases = _build_emitt_cases(tag, order_tag_str)
    ...
    for case in _select_cases(args.cases, emitt_cases_by_name):
