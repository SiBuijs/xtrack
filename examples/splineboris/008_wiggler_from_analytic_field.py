...
def _taylor_cos_coeffs(k_x: float, max_order: int):
    ...
    for m in range(0, max_order // 2 + 1):
        ...
        if n > max_order:
            break
        coeffs[n] = ((-1.0) ** m) * k_x**n / float(np.math.factorial(n))
    return coeffs
...
def main():
    ...
    # Choose a pole width; for simplicity set it equal to the period.
    pole_width = length / n_periods
    ...
    for ee, s_val in enumerate(s_grid):
        idx = min(int(s_val // (length / len(elements))), len(elements) - 1)
        el = elements[idx]
        s_local = s_val - idx * el.length
    ...
    plt.close("all")
    fig_field, (ax1, ax2) = plt.subplots(2, 1, sharex=True, figsize=(10, 6))
    ax1.plot(s_grid, by_analytic, label="analytic By", color="C0")
    ax1.plot(s_grid, by_spline, "--", label="spline By", color="C1")
    ax1.set_ylabel("By [T]")
    ax1.legend(loc="best")
    ax1.grid(True, alpha=0.3)

    ax2.plot(s_grid, by_spline - by_analytic, color="C3")
    ax2.set_xlabel("s [m]")
    ax2.set_ylabel("By_spline - By_analytic [T]")
    ax2.grid(True, alpha=0.3)
    fig_field.suptitle("SplineBoris wiggler field reconstruction at y=0, x=1e-4 m")
