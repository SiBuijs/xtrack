...
result. We also evaluate the one-turn transfer matrix and report its
symplectic deviation and determinant.
"""
...
plt.loglog(
    n_steps_vect[:-1],
    np.abs(sympl_error[0])
    * n_steps_vect[0] ** 2
    * 1
    / np.array(n_steps_vect[:-1]) ** (2),
    "--",
    label=r"~ 1/$N_\text{steps}^2$",
)
plt.xlabel("Number of steps")
